...
class JinaServer {
    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            try {
                ...
                const err = error;
                throw new McpError(ErrorCode.InternalError, `Jina API error: ${err.response?.data?.message || err.message}`);
            }
        });
    }
    ...
    validateCrossModalSearchArgs(args) {
    ...
    async handleSemanticSearch(args) {
        const response = await this.axiosInstance.post(`/collections/${args.collection}/search`, {
            query: args.query,
            limit: args.limit || 10,
            type: 'text',
        });
    ...
    async handleCrossModalSearch(args) {
        const response = await this.axiosInstance.post(`/collections/${args.collection}/search`, {
            query: args.query,
            limit: args.limit || 10,
